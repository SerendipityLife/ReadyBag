...
CSV 파일에서 상품 데이터를 읽어와 데이터베이스에 업로드합니다.
사용법: python products_upload_script.py <CSV파일경로>
"""
    ...
    try:
        ...
        with open(file_path, 'r', encoding='utf-8') as csv_file:
            ...
            for row in reader:
                ...
                # featured 필드 처리
                if product.get('featured') in ('TRUE', 'True', 'true', '1', 1):
                    product['featured'] = True
                else:
                    product['featured'] = False
                    
                # 해시태그 처리
                if not product.get('hashtags') or product.get('hashtags') == '':
                    product['hashtags'] = json.dumps([])
                elif not product['hashtags'].startswith('['):
...
def main():
    """메인 함수"""
    if len(sys.argv) < 2:
        print("사용법: python products_upload_script.py <CSV파일경로>")
        return
